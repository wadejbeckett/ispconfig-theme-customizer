<?php
/**
 * ispconfig-theme-customizer — SVG upload screening.
 * Copyright (c) 2026 Wade Beckett. MIT License — see the repo LICENSE.
 *
 * Split out of logo_upload.php so it can be exercised directly by
 * tests/svg/run.php without bootstrapping the ISPConfig app. logo_upload.php
 * requires this file; nothing else should.
 *
 * customizer_svg_ok($raw) is the entry point: true only if $raw is an SVG that
 * is safe to store and re-serve. The two helpers are exposed only because the
 * test corpus covers them individually.
 */
