<?php
/* ============================================================
 * classic — favicon.php  (the ICON reader for the stock design)
 * ------------------------------------------------------------
 * Serves the operator's own favicon, or the stock ISPConfig icon when none is
 * set. Both generated shell templates link THIS file as their <link
 * rel='icon'>, so the tab, the bookmark and the history entry carry the host's
 * brand instead of the ISPConfig mark. Companion to brand.php (colours + logo,
 * as CSS) and title.php (the tab title, as JS).
 *
 * classic ships NO assets of its own — every pixel comes from the stock
 * `default` theme — so "the design's shipped icon" here means stock's own icon
 * set, which is exactly what the shell linked before this endpoint existed
 * (install.sh pins classic's asset paths to themes/default/assets/). With
 * nothing stored, this design is byte-for-byte the panel you already had.
 *
 * That is also why this file is a near-copy of themes/clarity/favicon.php
 * rather than a shared include: it is a pre-authentication endpoint that must
 * keep working with the extension's module uninstalled, and each design owns
 * its own fallback. Only the fallback list below differs.
 *
 ...
 *
 * Resolution and both validators must stay identical to themes/clarity/
...
/* ---- what this design falls back to, tried in order --------------------
 * Crispest first, then the universally supported container. classic has no
 * assets directory of its own — themes/classic/ holds these three endpoints and
 * its docs, nothing else — so the fallback is the stock theme's icon set, the
 * very files the generated shell linked before this endpoint existed. If a
 * future ISPConfig moves them, favicon_send_shipped()'s last resort keeps the
 * response valid rather than 404ing on every page. */
$fallbacks = array(
    __DIR__ . '/../default/assets/favicon/favicon-32x32.png',
    __DIR__ . '/../default/assets/favicon/favicon.ico',
    __DIR__ . '/../default/assets/favicon/favicon-16x16.png',
);
